...
#define MAX_BT_ACCEPTORS 6 // the maximum ESP32 can handle is 7
...
static EventGroupHandle_t _spp_event_group_l[MAX_BT_ACCEPTORS] ={};
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_SRV_OPEN_EVT://Server connection open
        if (param->srv_open.status == ESP_SPP_SUCCESS) {
            log_i("ESP_SPP_SRV_OPEN_EVT: saved handle  %u", remote_nodes[current_client_id].handle);
            log_i("ESP_SPP_SRV_OPEN_EVT: opened handle %u", param->srv_open.handle);
...
static bool _stop_bt()
{
    if (btStarted()){
        for (int i=0; i<MAX_BT_ACCEPTORS; i++){
        if(remote_nodes[i].handle)
            esp_spp_disconnect(remote_nodes[i].handle);
...
{
    if(linkid>MAX_BT_ACCEPTORS){
        log_e("Error: Link id out of range. Please use a linkid between 0-%d",MAX_BT_ACCEPTORS);
        return -1;
    }
...
size_t BluetoothSerial::write(uint8_t c, int linkid)
{  
    if(linkid>MAX_BT_ACCEPTORS){
        log_e("Error: Link id out of range. Please use a linkid between 0-%d",MAX_BT_ACCEPTORS);
        return 0;
    }
    return write(&c, 1, linkid);
}

size_t BluetoothSerial::write(const uint8_t *buffer, size_t size, int linkid)
{
    if(linkid>MAX_BT_ACCEPTORS){
        log_e("Error: Link id out of range. Please use a linkid between 0-%d",MAX_BT_ACCEPTORS);
        return 0;
...
{
    if(linkid>MAX_BT_ACCEPTORS){
        log_e("Error: Link id out of range");
        return false;
    }
...
bool BluetoothSerial::isClosed(int linkid){
    ...
    return xEventGroupGetBits(_spp_event_group_l[linkid]) & SPP_CLOSED;
}
